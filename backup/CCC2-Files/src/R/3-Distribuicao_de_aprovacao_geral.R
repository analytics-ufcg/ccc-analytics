...
for(i in 1:(nrow(dados_validos))){
  if (dados_validos$num_Nota[i] == "F"){
    if (dados_validos$media[i] < 5) {
      dados_validos$aprovado_final[i] = "1. Reprovação"
    }else{
      dados_validos$aprovado_final[i] = "2. Aprovação"
    }
  }else{
...
# Plotando os gráficos de Distribuição da aprovação geral de alunos por disciplina 
#o <- subset(d_outros, final %in% c("sim"))
#d <- subset(d_dsc, final %in% c("sim"))
...
ggplot(d_outros, aes(x=disciplina, y=Taxa, fill = Aprovados)) + labs(title="Distribuição da aprovação de alunos - Outros departamentos", y="Taxa", x="Disciplina") + geom_bar(stat = "identity")+ coord_flip()+ facet_grid(. ~ periodo)  + scale_fill_manual(values = c("#B80000","#339900"))
ggplot(d_dsc, aes(x=disciplina, y=Taxa, fill = Aprovados)) + labs(title="Distribuição da aprovação de alunos - DSC", y="Taxa", x="Disciplina") + geom_bar(stat = "identity")+ coord_flip()+ facet_grid(. ~ periodo)  + scale_fill_manual(values = c("#B80000","#339900"))
